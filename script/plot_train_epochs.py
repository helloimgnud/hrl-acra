...
def main():
    parser = argparse.ArgumentParser(description="Plot final RAC and LRC across all training epochs.")
    parser.add_argument(
        "run_dirs", 
        type=str, 
    ...
    parser.add_argument(
        "--output", "-o",
        type=str,
        default="save/train_compare_plot.png",
        help="Path to save the output plot image (default: save/train_compare_plot.png)"
    )
    parser.add_argument(
        "--grid", 
        action="store_true", 
    ...
    for idx, run_dir in enumerate(run_directories):
        ...
        for ep_idx, file_path in enumerate(train_files):
            try:
                ...
                required_cols = ['success_count', 'v_net_count', 'total_time_revenue', 'total_time_cost']
                ...
                final_time_rev = df_arr['total_time_revenue'].iloc[-1]
                final_time_cost = df_arr['total_time_cost'].replace(0, 1).iloc[-1]
                lrc = final_time_rev / final_time_cost
                
                epoch_num = train_epochs[ep_idx]
                
                epochs.append(epoch_num)
                rac_values.append(rac)
                lrc_values.append(lrc)
        ...
        ax1.plot(epochs, rac_values, marker='o', markersize=3, label=run_name, color=color, linewidth=1.5, alpha=0.9)
        # Plot LRC on Subplot 2
        ax2.plot(epochs, lrc_values, marker='s', markersize=3, label=run_name, color=color, linewidth=1.5, alpha=0.9)
        
        print(f"  -> Run: {run_name} | Total Train Epochs: {len(epochs)}")
        print(f"     Final RAC: {rac_values[-1]:.2f}% | Max RAC: {max(rac_values):.2f}%")
        print(f"     Final LRC: {lrc_values[-1]:.4f} | Max LRC: {max(lrc_values):.4f}")
    ...
    ax2.set_ylabel("Long-term Revenue-to-Cost (LRC) Ratio", fontsize=11, fontweight='semibold')
    ax2.set_title("Long-term Revenue-to-Cost (LRC) Ratio over Training Epochs", fontsize=13, fontweight='bold', pad=10)
